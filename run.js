...
(function() {
	...
	a.object.extend(c.prototype, {
		...
		_move: function(n, j, m) {
			...
			;
			/*
			 * 但两个点重合时，f（step）为0，此时g会出错(被除数为0)，g＝NaN
			 * 图层上的marker会消失
			 * so sunny，修改了，代码如下：
			 */
			if(f == 0) {
				...
				var g = g = Math.round(i._getDistance(l, k) / f);
			}

			if(g < 1) {
				/**
				 * sunny
				 * 当g＝0的时候，等待5秒在移动，即前后两点相同时，不是直接跳过，而是原地等待
				 */
				setTimeout(function() {
					i._moveNext(++i.i);
				}, 5000);
		...
		_moveNext: function(e) {
			...
			if(f._path.length - 1 < 1) {
				f._path.push(f._path[e]);
			}

			if(e < this._path.length - 1) {
				//sunny 额外添加
				var spd = Math.ceil(map.getDistance(new BMap.Point(f._path[e].lng, f._path[e].lat), new BMap.Point(f._path[e + 1].lng, f._path[e + 1].lat)) / 5);
				console.log(spd)
				if(f._opts.speed != spd) {
					f._setOptions({
						speed: spd
					})
				}
